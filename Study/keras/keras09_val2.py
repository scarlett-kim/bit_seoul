# 검증하기 validation_split
#머신러닝한테 알아서 검증셋 분리해서 해라 
...
# x_val = np.array([11,12,13,14,15]) #주석한다 
...
#변경한다0.2로 train의 20%로 쪼개갰다 15개의 데이터면 3개(20% 검증데이터) x와 y는 동일하게 짤려서 검증된다 
# , validation_data=(x_val, y_val)) #꼭 valication을 분리해야하는가? 
